  ...
  <ul class="space-y-2">
    <li v-for="item in items" :key="item.label">
      <component
        ...
        :exact-active-class="'bg-gray-200 dark:bg-gray-700 dark:text-white'"
        @click="isShowing = !isShowing"
      >
        <component :is="item.icon" class="text-lg" />
        <span class="select-none" :class="{ 'md:hidden': collapsed }">{{ item.label }}</span>
        <BsArrowRightShort
          v-if="item.children"
